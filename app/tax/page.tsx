...
export default function TaxDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      let taxRate = totalSales <= 25_000_000 ? 0.2 : 0.3
      const taxOwed = netProfit > 0 ? netProfit * taxRate : 0

      setStats({ totalSales, totalCost, totalExpenses, netProfit, taxRate, taxOwed })
      setLoading(false)
    }
  ...
  const handleExport = () => {
    ...
    csv += `Stat,,Net Profit,,${stats.netProfit}\n`
    csv += `Stat,,Tax Rate,,${(stats.taxRate*100).toFixed(0)}%\n`
    csv += `Stat,,Tax Owed,,${stats.taxOwed}\n`
  ...
  if (loading) return (
    <ProtectedRoute>
      <div className="flex items-center justify-center h-screen text-gray-500 dark:text-gray-400">Loading...</div>
    </ProtectedRoute>
  )
          ...
          <>
            <div className="flex gap-2 text-sm mb-4">
              {["month","quarter","year"].map(p => (
                <PeriodButton key={p} active={period === p} onClick={() => setPeriod(p as any)}>{p.charAt(0).toUpperCase() + p.slice(1)}</PeriodButton>
              ))}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
              ...
              <StatCard title="Net Profit" value={`₦${stats.netProfit.toLocaleString()}`} />
              <StatCard title="Applicable Tax Rate" value={`${(stats.taxRate*100).toFixed(0)}%`} />
              <StatCard title="Estimated Tax Owed" value={`₦${stats.taxOwed.toLocaleString()}`} />
            </div>
          </>
        )}
